...
echo "[logstash-7.x]" > /tmp/elastic.repo
...
/usr/share/elasticsearch/bin/elasticsearch-setup-passwords auto -b > /etc/elasticsearch/passwords.txt

#### get the kibana_system password and add it to kibana
KIBANA_SYS_PASS=`grep -oE 'kibana_system = [a-zA-Z0-9]+' /etc/elasticsearch/passwords.txt |cut -d'=' -f 2 |tr -d ' '`
